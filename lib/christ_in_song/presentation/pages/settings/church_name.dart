...
class _ChurchNameSettingsState extends State<ChurchNameSettings> {
  ...
  void initState() {
    ...
    final churchNameBloc = context.read<ChurchNameBloc>();
    if (churchNameBloc.state is NameChanged) {
      _churchNameController.text =
          (churchNameBloc.state as NameChanged).churchName;
    }
  }
    ...
    return Scaffold(
      ...
      body: BlocBuilder<ThemeBloc, ThemeState>(
        builder: (context, themState) {
          var dynamicColor = themState.themeData.brightness == Brightness.dark;
          return Container(
                    ...
                    decoration: InputDecoration(
                      enabledBorder: OutlineInputBorder(
                        borderSide: BorderSide(
                          color:
                              dynamicColor ? AppColors.mainColor : Colors.black,
                        ), // Set the enabled border color to yellow when isDark is true, otherwise set to black
                      ),
                      focusedBorder: OutlineInputBorder(
                        borderSide: BorderSide(
                          color:
                              dynamicColor ? Colors.white : AppColors.mainColor,
